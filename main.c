...
char *do_expand(t_minishell *minishell, t_args *target)
{
    ...
    target_value = target->arg;
    token = ft_strdup(target_value + 1);
    if (!token)
        return (NULL);
    ...
    while(minishell->env[i])
    {
            ...
            && minishell->env[i][ft_strlen(token)] == '=')
        {
            ...
            free(target->arg);
            target->arg = env_value;
            free(token);
            printf("env value found : %s\n", target->arg);
            return (env_value);
        }
        i++;
    }
...
int parseur(t_minishell *minishell, t_args **args)
{
    t_args *current;
