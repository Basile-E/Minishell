...
/*
    Work in progress but the idea is :
        extract the token that we should look for
        if the token is found
            return his value
        else 
            return NULL; (comment return un char "vide")
    then during parsing :
        we take the ret value and set args.arg on it
*/

char *do_expand(t_minishell *minishell, t_args *target)
{
    char *expanded;
    char *target_value;
    char *token;
    int i;
    target_value = target->arg; // il faut une fonction qui extrait le token a chercher dans la linked list et idealement c'est ca qui sera la target value

    // extraction du token
    i = 1; // because target_value start with $ ($USER)
    while (target_value[i])
    {
        token[i - 1] = target_value[i];
        i++;
    }
    printf("debug : print of the token generated by do_expand\n%s\n", token);
